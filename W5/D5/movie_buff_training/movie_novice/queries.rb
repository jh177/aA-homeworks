...
def below_average_years
  ...
  Movie.where('score < 5').group('yr').select('COUNT(*) AS bad_movies, yr').order('bad_movies DESC')
end
...
def pulp_fiction_actors
  ...
  # select from actors join movies on actors.id = movies.actor_id
end
...
def uma_movies
  ...
  Actor.joins(:movies).where('actors.name = ?', 'Uma Thurman').select('movies.id, movies.title, movies.yr').order('movies.yr DESC')
end
